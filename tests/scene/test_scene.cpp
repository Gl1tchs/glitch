...
TEST_CASE("Scene entity relations") {
	...
	REQUIRE(e1.is_parent());
	REQUIRE(Entity::is_parent_of(e1, e3));
	REQUIRE(Entity::is_parent_of(e1, e4));
	REQUIRE(!Entity::is_parent_of(e1, e5));

	REQUIRE(e4.get_parent() == e1);

	REQUIRE(e2.get_children().front() == e5);
	REQUIRE(e5.get_parent() == e2);
}
...
TEST_CASE("Scene copy") {
	...
	REQUIRE(e1_copy != INVALID_ENTITY);
	REQUIRE(e2_copy != INVALID_ENTITY);
	REQUIRE(e1_copy.get_uid() == e1.get_uid()); // UID preservation check

	REQUIRE(e1_copy.is_parent());
	REQUIRE(e1_copy.get_children().size() == 2);

	REQUIRE(e2_copy.is_parent());
	REQUIRE(e2_copy.get_children().size() == 1);

	// Verify relationship mapping in the new scene
	REQUIRE(e3_copy.get_parent() == e1_copy);
	REQUIRE(e5_copy.get_parent() == e2_copy);
}

TEST_CASE("Scene Serialization Round-Trip") {
	os::setenv("GL_WORKING_DIR", std::filesystem::temp_directory_path().string().c_str());
	...
	REQUIRE(Scene::serialize(scene_filename, src_scene));

	auto scene_path = AssetSystem::get_absolute_path(scene_filename);
	REQUIRE(scene_path.is_ok());
	REQUIRE(std::filesystem::exists(scene_path.value()));
	...
	SECTION("Verify Entity Existence and IDs") {
		REQUIRE(dst_scene->exists(e1.get_uid()));
		REQUIRE(dst_scene->exists(e2.get_uid()));
		REQUIRE(dst_scene->exists(e3.get_uid()));

		// Should be 3 entities
		// (Assuming view() or a count method exists, otherwise verify by lookups)
		// REQUIRE(dst_scene->entity_count() == 3);
	}

	SECTION("Verify Hierarchy") {
		...
		REQUIRE(e1_loaded.is_parent());
		REQUIRE(e2_loaded.get_parent() == e1_loaded);
		REQUIRE(e3_loaded.get_parent() == e1_loaded);

		// Ensure E2/E3 are strictly siblings
		REQUIRE(e2_loaded.get_parent() == e3_loaded.get_parent());
	}

	SECTION("Verify Components") {
		Entity e2_loaded = dst_scene->find_by_id(e2.get_uid()).value();
		Entity e3_loaded = dst_scene->find_by_id(e3.get_uid()).value();

		// Check simple component existence
		REQUIRE(e2_loaded.has_component<PointLight>());
		REQUIRE(!e2_loaded.has_component<DirectionalLight>()); // Should not have parent's comp
		REQUIRE(e3_loaded.has_component<CameraComponent>());
	}

	SECTION("Verify Asset References") {
		Entity e1_loaded = dst_scene->find_by_id(e1.get_uid()).value();
		if (e1_loaded.has_component<MockComponent>()) {
			...
			REQUIRE(mc->handle.is_valid());
		}
	}

	if (std::filesystem::exists(scene_path.value())) {
		std::filesystem::remove(scene_path.value());
	}
