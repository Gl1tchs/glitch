...
Image VulkanRenderBackend::image_create(DataFormat p_format, glm::uvec2 p_size, const void* p_data,
	...
	if (!p_data) {
		...
		command_immediate_submit(
				[&](CommandBuffer p_cmd) {
					...
					command_copy_buffer_to_image(p_cmd, staging_buffer, new_image, copy_view);
