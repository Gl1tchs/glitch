...
void VulkanRenderBackend::command_begin_rendering(CommandBuffer p_cmd,
		const glm::uvec2& p_draw_extent, VectorView<RenderingAttachment> p_color_attachments,
...
void VulkanRenderBackend::command_begin_render_pass(CommandBuffer p_cmd, RenderPass p_render_pass,
		FrameBuffer framebuffer, const glm::uvec2& p_draw_extent, Color p_clear_color) {
...
void VulkanRenderBackend::command_bind_uniform_sets(CommandBuffer p_cmd, Shader p_shader,
		uint32_t p_first_set, VectorView<UniformSet> p_uniform_sets, PipelineType p_type) {
...
void VulkanRenderBackend::command_set_viewport(CommandBuffer p_cmd, const glm::uvec2& size) {
	VkViewport viewport = {
		.x = 0,
		.y = 0,
		.width = (float)size.x,
...
void VulkanRenderBackend::command_set_scissor(
		CommandBuffer p_cmd, const glm::uvec2& p_size, const glm::uvec2& p_offset) {
	VkRect2D scissor = {};
