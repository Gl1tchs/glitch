...
public:
	...
	void init(std::shared_ptr<Window> p_window) override;
	...
	RenderPass render_pass_create(VectorView<RenderPassAttachment> p_attachments,
	...
	FrameBuffer frame_buffer_create(RenderPass p_render_pass, VectorView<Image> p_attachments,
	...
	void swapchain_resize(CommandQueue p_cmd_queue, Swapchain p_swapchain, glm::uvec2 size,
	...
	UniformSet uniform_set_create(
			VectorView<ShaderUniform> p_uniforms, Shader p_shader, uint32_t p_set_index) override;
	...
	void command_begin_rendering(CommandBuffer p_cmd, const glm::uvec2& p_draw_extent,
			VectorView<RenderingAttachment> p_color_attachments, Image p_depth_attachment) override;
	...
	void command_bind_uniform_sets(CommandBuffer p_cmd, Shader p_shader, uint32_t p_first_set,
			VectorView<UniformSet> p_uniform_sets,
	...
	void command_copy_image_to_image(CommandBuffer p_cmd, Image p_src_image, Image p_dst_image,
			...
			uint32_t p_src_mip_level = 0, uint32_t p_dst_mip_level = 0) override;
	...
	void imgui_init_for_platform(GLFWwindow* p_glfw_window, DataFormat p_color_format) override;
